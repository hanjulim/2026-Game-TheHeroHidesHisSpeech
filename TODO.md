...
- [x] 캐릭터/적 렌더 함수 이동
- [x] HUD 렌더 함수 이동
- [x] 타깃 제스처/다음 제스처/비트 바 렌더 함수 이동
- [x] 컷신/엔딩/오버레이 렌더 분기 이동
- [x] `gameLoop()`에서 그리기 호출만 남도록 정리
- [ ] 렌더 분리 후 시각 연출 차이가 없는지 확인

Phase 8 메모

- [x] `createRenderer()`를 기준으로 장면 렌더링 책임을 `js/render.js`로 이동
- [x] `main.js`는 상태 업데이트와 렌더 호출 조립만 담당
- [x] 향후 배경/캐릭터를 이미지 에셋으로 바꿀 때 `render.js` 내부 구현만 교체할 수 있는 구조로 정리
- [ ] 브라우저 실기동으로 시각 연출과 레이아웃 최종 확인
